...
new_glass1 <- subset(glass, glass$Si<74)
new_glass1 <- subset(new_glass1, glass$Si != max(outliers))

boxplot(new_glass1$Si)
max(boxplot.stats(new_glass1$Si)$out)
```
# Test 2 Remove all outliers(Currently our best option)
```{r include=T, echo=T}
new_glass2 <- subset(glass, glass$Si<74)
new_glass2 <- subset(new_glass2, new_glass2$Si>71.5)
boxplot(new_glass2$Si)
```
...
# Test 4: Check histogram for normality
...
shapiro.test(2^(glass$Si))

shapiro.test(sqrt(glass$Si))

shapiro.test(sqrt(max(glass$Si+1) - glass$Si))

shapiro.test(log(glass$Si))
...
I dont think we need to transform data since no transformation actual gets us to < 0.05. I think at this point we need to do non-parametric test.
...
# Part (e): perform simulation analysis
