...
$$
\begin{aligned}
\frac{d \ln L}{d \sigma^2} &= \frac{d}{d \sigma^2} {\left(-\frac{n}{2}\ln{(2\pi)} - \frac{n}{2}\ln{(\sigma^2)} -\frac{1}{2\sigma^2}\sum\limits_{i=1}^{n}{(x_i - \mu)^2}\right)} \\
&= -\frac{n}{2\sigma^2} - {\left[  \frac{1}{2}\sum\limits_{i=1}^{n}{(x_i - \mu)^2}\right]}\frac{d}{d \sigma^2}\left(\frac{1}{\sigma^2}\right) \\
&= -\frac{n}{2\sigma^2} - {\left[  \frac{1}{2}\sum\limits_{i=1}^{n}{(x_i - \mu)^2}\right]}\left(-\frac{1}{(\sigma^2)^2} \right) \\
&= \frac{1}{2\sigma^2}\left[ \frac{1}{\sigma^2}\sum\limits_{i=1}^{n}{(x_i - \mu)^2} -n \right]
\end{aligned}
$$


### Step 5: Solving for the desired parameter.


$$
\begin{aligned}
\frac{d \ln L}{d \sigma^2} &= 0 \\
\frac{d \ln L}{d \sigma^2} &= \frac{1}{2\sigma^2}\left[ \frac{1}{\sigma^2}\sum\limits_{i=1}^{n}{(x_i - \mu)^2} -n \right] = 0 \\
&=  \frac{1}{\sigma^2}\sum\limits_{i=1}^{n}{(x_i - \mu)^2} -n  = 0 \\
&= \frac{1}{n}\sum\limits_{i=1}^{n}{(x_i - \mu)^2} = \sigma^2 \\
\end{aligned}
$$

Therefore the estimator $\hat{\sigma}$ is equal to the unadjusted sample variance.

$$
\hat{\sigma} = \sqrt{\frac{1}{n}\sum\limits_{i=1}^{n}{(x_i - \hat{\mu})^2}}
$$

Since $\hat{\mu}= \bar{x}$ then the estimator $\hat{\sigma}$ is equal to,

$$
\hat{\sigma} = \sqrt{\frac{1}{n}\sum\limits_{i=1}^{n}{(x_i - \bar{x})^2}}
$$
