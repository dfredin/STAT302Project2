...
min.RSS <- function(data, par) {
              with(data, sum((par[1] + par[2] * Na + par[3] * Mg + par[4] * Al 
                              + par[5] * K + par[6] * Ca + par[7] * Ba + par[8] * Fe - Si)^2))
}
...
ols_test_breusch_pagan(final_model) # violated

ols_plot_resid_stud(final_model) # Outlier exist
```

Based on the rainbow test on our model, the linearity assumption is not violated, as the null hypothesis of a linear fit is not rejected (p > 0.05). However, the results of a Shapiro test on the residuals between our final model and training data conclude they are not normal, as p < 0.05 here. On the other hand, the results of a shapiro test on the residuals between our final model and the testing data conclude that the residuals for our testing are normal as p = 0.605 > 0.05. Heteroscedasticity is also present in the residuals, as is an outlier. It may
be an extreme value from our train data set.Thus, the assumptions on residuals of regression models on train data to be normal do not hold.
...
```
As we can see, the Si contents that we predicted using the simulation method
have the range between 65.79 and 80.43. The median value of our simulation 
predict Si is 72.43 while the mean is 72.56. The interquartile range of the 
simulation predicted Si value is 74.44 - 71.00  = 3.44. 

We choose to use normal distribution method to generate the simulated data 
for all the independent variables because our train data is approximately
...
$$
\begin{aligned}
\Large Si &= \Large  99.01 - (0.96 \cdot Na) - (0.99\cdot Mg) - (0.994 \cdot Al) \\ &- \Large (1.13 \cdot K) - (0.97 \cdot Ca) - (0.77 \cdot Ba) + (1.3 \cdot Fe)
\end{aligned}
$$

This regression equation suggests that when none of the critical elements are present in the glass, 98.82% of the material will be Si. The negative slopes in the equation means that an increase of the presence of any other element by one percent will decrease the Si content of the glass by roughly one percent, which makes sense. 
...
that are silicon in the glass will increases by 1.3%. (Certain chemical properties?)
...
```
Based on the rainbow test on our model, the linearity assumption is not violated, as the null hypothesis of a linear fit is not rejected (p > 0.05). The result of a Shapiro test on the residuals between the predictions of our simulated model and simulated data conclude that they are normal, as p > 0.05.  It may be an extreme value from our train data set.Thus, the assumptions on residuals between simulated data and real data are normally distributed do not hold. 
