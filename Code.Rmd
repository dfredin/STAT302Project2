...
```
We choose the variable RI(reflective inference) and Si as our x and y axis to
...
Based on the RMSE values we generated for the four models above, the model 3
...
```
We can clearly sees that the two group of different glass products WinF and
WonF
...
y <- e + coef[1] + coef[2]*Na + coef[3]*Mg + coef[4]*Al + coef[5]*K + coef[6]*Ca + coef[7]*Ba + coef[8]*Fe
