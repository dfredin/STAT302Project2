...
boxplot(glass_filtered$Si)
max(boxplot.stats(glass_filtered$Si)$out)
```
# Test 2 Remove all outliers(Currently our best option)
```{r include=T, echo=T}
glass_filtered <- subset(glass, glass$Si<74)
glass_filtered <- subset(glass_filtered, glass$Si>71.5)
boxplot(glass_filtered$Si)
```
...
```{r}
ggplot(data=glass, aes(x=type, y=Si)) + 
  geom_boxplot()
```
...
glass_model3 <- lm(sqrt(Si)  ~ Na + Mg + Al + K + Ca + Ba + Fe + type , data = mutate_glass)
...
# Training and testing / k cross-validation method
```{r include=T, echo=T} 
set.seed(123)
